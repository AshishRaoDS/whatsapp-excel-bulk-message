...
| 2026-02-27 | Added per-recipient template parameters: template mode now supports "Use message from Excel" toggle. When enabled, reads phone+message columns and sends each row's message as the {{1}} template parameter. Users need a custom Meta template with {{1}} placeholder (hello_world doesn't support params). UI updated with toggle, warnings, and setup guidance. |
